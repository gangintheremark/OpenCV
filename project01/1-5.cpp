...
void main() {
	...
	for (int i = 0; i < image.rows; i++) {  // 히스토그램 생성
	...
	int cumulative_histo[256] = { 0, };
	int sum = 0;

	for (int i = 1; i < 256; i++) { // 누적 히스토그램 
		sum += histo[i];
		cumulative_histo[i] = sum;
	}

	float normalized_cumulative_histo[256] = { 0.0, };  // 정규화된 누적 히스토그램 
